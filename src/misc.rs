...
pub fn generate_frame(path: &Path) -> Result<Vec<objects::Frame>, serde_json::error::Error> {
    ...
    let f = objects::Frame { spheres: spheres, spherocylinders: vec![], text: String::new() };
...
pub fn load_toml<T: Decodable>(path: &Path) -> Result<T, Box<Error>> {
    ...
    let values = match parsed {
        ...
        None => {
            ...
            let err: Box<Error> = parser.errors.pop().expect(
                "Toml parser failed, yet provided no error. Unsure how to proceed."
                ).into();
            return Err(err)
        }
    };
...
pub fn draw_cube(window: &mut Window) -> kiss3d::scene::SceneNode {
    ...
    for rot in &rotations {
        for t in &translations {
            ...
            match *rot {
                Some(r) => {
                    caps.append_rotation(&(r * std::f32::consts::FRAC_PI_2))
                }
                None => {}
            }
